...
To track the real-time health of the loan portfolio and audit automated decisions, portfolio metrics are aggregated into an executive Power BI Desktop interactive dashboard.

### Credit Risk Dashboard Canvas
<img src="./Screenshot (31).png" alt="Power BI Credit Risk Assessment Dashboard" width="100%"/>

### Key Portfolio Performance Indicators (KPIs)
* **Volume Analysis**: Tracks total processed items (**22 total applications**).
* **Financial Exposure**: Aggregates portfolio capital demands (**$1M Sum of Annual Income**, **$489K Sum of Loan Amount Requested**).
* **Risk Baselines**: Visualizes credit portfolio health summaries (**614.55 Average Credit Score**, **0.45 Average Debt-to-Income Ratio**).
* **Segment Breakdown**: Features advanced visualization modules including a System Initial Action donut chart, Credit Score distributions by Application ID, and interactive slicers for rapid auditing based on risk level, status, or system remarks.
...
* **Power BI Desktop** installed locally to review or modify the dashboard reports.
* Google Workspace Account with **Google Sheets API** and **Gmail API** scopes configured via OAuth2 credentials.

### Configuration Steps
1. **Reporting Setup**: Open your Power BI dashboard file and update the data source connection settings to pull directly from your production Google Sheet ledger.
